...
L.Control.FuseSearch = L.Control.extend({
    ...
    _createPanel: function(map) {
        ...
        L.DomUtil.create('i', 'search-image fas fa-search-location', header);
        ...
        // Close button
        var close = this._closeButton = L.DomUtil.create('a', 'close-search', header);
    ...
    createResultItem: function(props, container, popup) {
        ...
        if (null !== this.options.showResultFct) {
            ...
            for (var i = 1; i < this._keys.length; i++) {
                if (props[this._keys[i]] !== undefined && props[this._keys[i]] != '') {
                    str += '<br/> <span>' + this._keys[i]+': </span>' + props[this._keys[i]];
                }
            }
